...
let activeServiceFeeTiers = DEFAULT_SERVICE_FEE_TIERS.map((tier) => ({ ...tier }));
...
export const setServiceFeeTiers = (tiers) => {
  activeServiceFeeTiers = normalizeServiceFeeTiers(tiers);
  return activeServiceFeeTiers;
};

export const getServiceFeeTiers = () => activeServiceFeeTiers;
...
export const calcServiceFeeAmount = (ticketAmount, tiers = activeServiceFeeTiers) => {
...
export const buildServiceFeeTierLines = (tiers = activeServiceFeeTiers) =>
  normalizeServiceFeeTiers(tiers).map((tier) => formatServiceFeeTierLabel(tier));

export const serviceFeeTiersToFormRows = (tiers = activeServiceFeeTiers) =>
...
export const fetchServiceFeeTiers = async (apiBase) => {
  const base = String(apiBase || "").replace(/\/+$/, "");
  if (!base) {
    return setServiceFeeTiers(DEFAULT_SERVICE_FEE_TIERS);
  }
  try {
    const response = await fetch(`${base}/service-fee/tiers`);
    if (!response.ok) {
      return getServiceFeeTiers();
    }
    const data = await response.json();
    if (Array.isArray(data.tiers)) {
      return setServiceFeeTiers(data.tiers);
    }
  } catch {
    /* keep current tiers */
  }
